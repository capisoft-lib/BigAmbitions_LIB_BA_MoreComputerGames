# Publication Steam Workshop — MCG 1.0.1
...
| Version | **1.0.1** — assembly **1.0.1.0** |
...
Les descriptions présentent les raccourcis configurables **Retour au menu** et **Quitter**, le correctif de découverte des jeux Workshop séparés, le nettoyage sûr à la fermeture, les 22 langues et les records locaux. Le leaderboard reste une intention, sans date confirmée, pas une fonctionnalité de la 1.0.1.
...
- Sur une copie de partie, vérifier le menu du moniteur, un jeu vanilla et un jeu MCG installé uniquement depuis le Workshop, les raccourcis par défaut et reconfigurés, la pause Escape, le changement de langue, la persistance d'un record et la sortie vers le menu principal sans exception. Le [rapport de vérification](../../VERIFICATION.md) distingue les tests déjà effectués des essais natifs encore à faire.
...
Les fiches des jeux compatibles doivent déclarer **MCG** dans leurs Required Items et leurs DLL doivent référencer l'assembly **1.0.1.0**, sans l'embarquer. Leur classe `RegisterModClass` doit rester chargeable avec BAModAPI seul.
