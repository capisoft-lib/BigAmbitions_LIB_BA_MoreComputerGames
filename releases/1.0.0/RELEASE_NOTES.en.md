...
- The original **Leave** button keeps its translated caption and adds **[TAB]** throughout the MCG menu and games. Its action is unchanged; the hint and temporary text sizing are restored when the session ends.
...
**For game authors:** rebuild compatible game mods against MCG **1.0.0**, assembly **1.0.0.0**. The native loader can reject binaries that still reference major version 0, despite the preserved API signatures. Ship MCG as a separate Workshop dependency; do not bundle its DLL.
